...
import { expectPreflightReady, signIn, waitSaved } from "./helpers";
...
test("proposal printing fails closed until bid preflight is ready", async ({ page }) => {
  const runtimeErrors: string[] = [];
  page.on("pageerror", (error) => runtimeErrors.push(error.message));

  await signIn(page);
  await page.getByLabel("Project name", { exact: true }).fill(`Guarded proposal ${Date.now()}`);
  await page.getByRole("button", { name: "Create project" }).click();
  await expect(page).toHaveURL(/\/project\//);

  await page.getByRole("button", { name: "scope" }).click();
  const printButton = page.getByTestId("print-proposal-btn");
  const printGate = page.getByTestId("proposal-print-gate");
  const preview = page.getByTestId("proposal-preview");

  await expect(printButton).toBeDisabled();
  await expect(printButton).toHaveText(/Resolve blockers to print/);
  await expect(printGate).toContainText("Proposal printing is blocked");
  await expect(printGate).toContainText("The bid has no priced scope");

  // Browser-level Ctrl+P/menu printing must not bypass the disabled button.
  await page.emulateMedia({ media: "print" });
  await expect(printGate).toBeVisible();
  ...
  const amount = page.getByLabel("Amount for Permit allowance");
  ...
  await expect(printButton).toHaveText(/Print proposal/);
  await expect(printGate).toHaveCount(0);
